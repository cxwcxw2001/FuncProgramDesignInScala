...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = b.b1.row == goal.row & b.b1.col == goal.col && b.isStanding
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    (
      for (ln <- b.legalNeighbors) yield (ln._1, ln._2 :: history)
      ).to(LazyList)
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(List((startBlock, Nil)).to(LazyList), Set())
